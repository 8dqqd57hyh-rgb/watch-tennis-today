...
export default async function AtpLiveTodayPage() {
  ...
  const liveAtpMatches = matches
    .filter((match) => {
      return isAtpMatch(match) && isLiveMatch(match.status);
    })
    .sort((a, b) => statusPriority(a.status) - statusPriority(b.status));
  const suspendedAtpMatches = matches.filter((match) => isAtpMatch(match) && isSuspendedMatch(match.status));
...
<a href="/best-ways-to-watch-tennis-online" className="rounded-2xl border border-zinc-700 px-6 py-4 font-black">How to Watch</a>
        ...
        {suspendedAtpMatches.length > 0 ? (
          <section className="mb-14">
            <h2 className="mb-6 text-4xl font-black">Suspended ATP matches</h2>
            <div className="space-y-5">{suspendedAtpMatches.map((match) => (
